...
import {
  ...
  Image as ImageIcon, ChevronLeft, ChevronRight, Lightbulb, Volume2
} from 'lucide-react';
import { supabase } from '../../lib/supabase';
import { uploadCloudinaryAsset } from '../../services/cloudinaryUpload';
import { requestAIJson } from '../../services/aiService';
import { speakKo, playCorrectSound, playIncorrectSound } from '../../services/audioService';
...
export function ShareCodeBox({ code, onCreateAnother }) {
  ...
  const shareText = `Mình vừa tạo một bài học tiếng Hàn trên app! Vào mục Cộng đồng > Bài học tự tạo, bấm "Duyệt bài học" rồi nhập mã: ${code}`;
  ...
  return (
    <div className="cl-saved-box">
      ...
      <p>Đã tạo bài học! Gửi mã này cho bạn bè để họ học đúng bài của bạn:</p>
      ...
      <button className="cl-post-btn" onClick={onCreateAnother}>Tạo bài khác</button>
    </div>
  );
}
...
export function CustomLessonHub({ profile, onStudy }) {
  ...
  const loadSavedCodes = async () => {
    try {
      ...
      const { data, error } = await supabase
        ...
        .select('lesson_id, custom_lessons(id, code, creator_id, title, words, quiz_types, attachments, created_at, profiles!custom_lessons_creator_id_fkey(display_name))')
  ...
  const loadLessons = async () => {
    try {
      ...
      const { data, error } = await supabase
        .from('custom_lessons')
        .select('id, code, creator_id, title, words, quiz_types, attachments, created_at, profiles!custom_lessons_creator_id_fkey(display_name)')
        .eq('status', 'visible')
        .order('created_at', { ascending: false });
      if (error) throw error;
  ...
  const updateWord = (i, field, val) => setWords((ws) => ws.map((w, idx) => (idx === i ? { ...w, [field]: val } : w)));
  const addWordRow = () => setWords((ws) => [...ws, { ko: '', vi: '', img: '', showImg: false }]);
  ...
  const saveLesson = async () => {
    ...
    try {
      ...
      setGenError('Không lưu được bài học lúc này (có thể do mất kết nối) — vui lòng thử bấm Lưu lại.');
    }
    setGenStep('');
    setSaving(false);
  };
  ...
  const findByCode = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .maybeSingle();
      if (error) throw error;
      if (data) onStudy(mapCustomLessonRow(data));
  ...
  return (
    <div className="cl-hub">
      <div className="cg-tabs mini">
        <button className={`cg-tab ${subTab === 'browse' ? 'on' : ''}`} onClick={() => setSubTab('browse')}>Duyệt bài học</button>
        <button className={`cg-tab ${subTab === 'create' ? 'on' : ''}`} onClick={() => setSubTab('create')}>+ Tạo bài học mới</button>
      </div>
      ...
      {subTab === 'browse' ? (
        <>
          ...
          {savedLessons === null ? null : savedLessons.length > 0 && (
            <>
              <p className="cg-sub" style={{ marginTop: 4 }}>📁 Sổ tay của tôi — bài đã tạo hoặc đã lưu:</p>
          ...
          {lessons === null ? (
            ...
            <div className="cg-empty"><SwBunnyEmpty /><p>Chưa có bài học nào được chia sẻ — hãy tạo bài đầu tiên!</p></div>
          ) : (
            <div className="cl-list">
              {lessons.map((l) => (
                <div key={l.code} className="cl-item">
        ...
        <div className="cl-create-form">
          ...
          {(() => {
            ...
            const missing = [];
            if (!title.trim()) missing.push('tên bài học');
            ...
            return (
              <div className="cl-find-err" style={{ background: '#FFF8E6', borderColor: '#F5D98A', color: '#9A7B1E' }}>
                Cần điền {missing.join(' và ')} thì mới bật được nút Lưu.
              </div>
            );
          })()}

          <button className="cg-post-btn cl-save-btn" onClick={saveLesson} disabled={!title.trim() || words.filter((w) => w.ko.trim() && w.vi.trim()).length < 2 || saving}>
            {genStep === 'ai' ? (<><Sparkles size={14} /> AI đang soạn câu ví dụ & bài kiểm tra...</>) : genStep === 'saving' ? 'Đang lưu...' : 'Lưu & Lấy mã chia sẻ'}
          </button>
        </div>
      )}
    </div>
...
export function CustomLessonStudyView({ lessonData, onBack, onStartQuiz }) {
  ...
  return (
    <section className="rv-page">
      ...
      <p className="cg-sub" style={{ marginTop: 0 }}>Bài học của {lessonData.author}</p>

      {hasQuiz && (
        <button className="cl-quiz-launch" onClick={() => onStartQuiz(lessonData)}>
          <Sparkles size={14} /> Làm bài kiểm tra AI tạo sẵn cho bài này
        </button>
      )}
