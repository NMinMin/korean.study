...
type AdminUser = { id: string; email?: string; display_name?: string; role: 'user' | 'admin'; level?: number; xp?: number; emailConfirmedAt?: string; lastSignInAt?: string }
...
function AdminSelect({ value, options, onChange, label }: { value: string; options: { value: string; label: string }[]; onChange: (value: string) => void; label?: string }) {
  ...
  return <div className={`admin-combobox ${open ? 'open' : ''}`} ref={rootRef}>
    <button type="button" className="admin-combobox-trigger" aria-label={label} aria-haspopup="listbox" aria-expanded={open} onClick={() => setOpen((current) => !current)} onKeyDown={(event) => { if (event.key === 'ArrowDown') { event.preventDefault(); move(1); setOpen(true) } if (event.key === 'ArrowUp') { event.preventDefault(); move(-1); setOpen(true) } if (event.key === 'Escape') setOpen(false) }}><span>{selected?.label}</span><ChevronDown size={17} /></button>
    ...
    <main className="admin-page">
      ...
      {tab === 'dashboard' && dashboard && <div className="admin-dashboard">
        ...
        <div className="admin-stat-grid">
          <article className="admin-stat purple"><span><GraduationCap size={25} /></span><div><small>Học viên đang hoạt động</small><strong>{dashboard.activeUsers}</strong></div><em><TrendingUp size={14} /> {range} ngày</em></article>
      ...
      {tab === 'users' && <><div className="admin-panel-title"><div><h2>Người dùng</h2><p>Chỉ admin mới có thể xem danh sách và cấp quyền quản trị.</p></div></div><div className="admin-table-wrap"><table><thead><tr><th>Học viên</th><th>Email</th><th>Vai trò</th><th>Cấp / XP</th><th>Đăng nhập gần nhất</th></tr></thead><tbody>{users.map((user) => <tr key={user.id}><td><b>{user.display_name || 'Người học'}</b></td><td>{user.email || '—'}</td><td><AdminSelect value={user.role} options={[{ value: 'user', label: 'Học viên' }, { value: 'admin', label: 'Admin' }]} label={`Vai trò của ${user.display_name || user.email}`} onChange={(value) => void updateRole(user.id, value as 'user' | 'admin')} /></td><td>Lv. {user.level || 1} · {user.xp || 0} XP</td><td>{user.lastSignInAt ? new Date(user.lastSignInAt).toLocaleString('vi-VN') : 'Chưa có'}</td></tr>)}</tbody></table></div></>}
      </section>}
